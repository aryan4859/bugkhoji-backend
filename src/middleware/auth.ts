...
export async function authenticate(
  ...
  try {
    ...
    logger.info(`User ${user.id} authenticated successfully from IP: ${clientIp}`)
    next()
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      if (!allowedRoles.includes(req.user.role)) {
        logger.warn(`Authorization failed: User ${req.user.id} with role ${req.user.role} attempted to access endpoint requiring roles: ${allowedRoles.join(", ")}`)
        res.status(403).json({ 
