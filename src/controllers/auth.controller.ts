...
import { generateAccessToken, generateRefreshToken, verifyRefreshToken, invalidateRefreshToken } from "../utils/token"
...
export async function login(req: Request, res: Response) {
  try {
    ...
    if (!user || !user.isActive) {
      logger.warn(`Login attempt for non-existent or inactive user: ${validatedData.email}`)
...
export async function refreshToken(req: Request, res: Response) {
  try {
    ...
    const decoded = jwt.decode(req.body.refreshToken || refreshToken) as { id: string } | null
...
export async function changePassword(req: Request, res: Response) {
  try {
    ...
    const passwordMatch = await bcrypt.compare(validatedData.currentPassword, user.passwordHash)
